...
export function createMemories({host,caption,message,onControls}){
  ...
  function show(){
    ...
    else{el.playsInline=true;el.controls=true;el.preload='auto';el.muted=muted||!!audio;el.onended=()=>{if(valid())next();};el.onplaying=()=>{if(valid()){clearTimeout(watchdog);message('');}};el.onwaiting=()=>{if(valid()){clearTimeout(watchdog);watchdog=setTimeout(fail,60000);}};el.onloadeddata=()=>{if(valid())el.play().catch(()=>{clearTimeout(watchdog);message('Tap the video to play');});};}
    el.src=a.src;
  }
