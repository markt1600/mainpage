...
A single-page daily dashboard in an editorial / broadsheet style: weather for Singapore, Tokyo, Vancouver, and Whistler, live market quotes, AI-written tech & lifestyle news with links to the original articles, Strava mileage, and a quote of the day — gathered fresh each morning.
...
| Birthdays (owner-only, shown 7 days before → 3 days after in the For Mark section) | encrypted store `data/birthdays.enc.json` — edit at **`/admin`** | Login |
...
- **`api/admin.js`** — backend for the admin editor. Auth: the owner's Google-login session, or the shared automation secret (`ADMIN_SECRET` / `DASHBOARD_SECRET`) for scripted access. `GET` returns the current lists; `POST` commits an edited list to `data/*.json` via the GitHub Contents API (needs `GITHUB_TOKEN`). Each save is a commit, so git history *is* the audit trail; the current blob `sha` is round-tripped so a concurrent edit 409s instead of silently clobbering.
- **`data/events.json`** — the authoritative Events list the dashboard fetches. Edited through `/admin`; the inline array in `index.html` remains as an offline/local-preview fallback. Birthdays live in the **encrypted** `data/birthdays.enc.json` (personal info — owner-only, served via `/api/private-events`).
...
- **`api/special.js`** — serves MERIDIAN's owner-only special editions. Specials live encrypted at rest in the public dailymag repo (X25519 sealed-box `.enc` blobs); this endpoint fetches a blob from raw.githubusercontent, decrypts it with `MERIDIAN_SPECIALS_KEY`, and serves it only to a valid owner session (the `.marktan.ai` `mt_owner` cookie or `?token=`). Everyone else gets a plain 404 — never a 401 — so the specials don't observably exist. PDFs over ~4.4 MB get a "delivered to your reMarkable" note instead (Vercel response cap).
- **`api/todos.js`** — Mark's private to-do list, encrypted at rest exactly like the private calendar (`data/todos.enc.json`, AES-256-GCM). `GET` returns the list + sha; `POST {todos, sha}` saves with a conflict guard; `POST {add: "text"}` is a sha-free server-side append used by the dashboard's quick-add box, the `/todo` voice page, and phone/watch shortcuts. Auth: owner session or admin secret. **Shortcuts must call `https://www.marktan.ai/...`** — the apex domain redirects to `www`, and Apple Shortcuts drops the POST body when following that redirect (the request degrades to a harmless GET).
- **`api/transcribe.js`** — voice → text for `/todo`'s hold-to-record button, via ElevenLabs Speech-to-Text (Scribe; reuses `ELEVENLABS_API_KEY`). The browser records a clip and POSTs it; the transcript comes back and is added via `/api/todos`.
...
- **`admin.html`** (served at **`/admin`**) — a self-contained CRUD editor: add/edit/delete rows in tables, then Save. Opens only for the owner's Google login (no password UI, no token deep-links). `noindex`.
...
Editing Birthdays and Events no longer means touching `index.html`. Visit **`/admin`** — it opens only for the owner's Google login (sign in on the dashboard first); there is no password. Edit both lists in a table UI. There's also a direct link in the **For Mark · Private** section. **Saving commits to GitHub** (`data/events.json` in plaintext; birthdays into the encrypted `data/birthdays.enc.json`) — which is the version history — and the change goes live once Vercel finishes the redeploy triggered by that commit (~a minute). The dashboard fetches events at load (inline-array fallback); birthdays arrive only with the owner login.

Requires two env vars (below): `ADMIN_SECRET` (the API automation secret) and `GITHUB_TOKEN` (fine-grained, **Contents: Read & Write** on `markt1600/mainpage`) so the function can read and commit. If neither `ADMIN_SECRET` nor `DASHBOARD_SECRET` is set, the admin APIs refuse to run — nothing ever opens unprotected.
...
| `DASHBOARD_SECRET` | `balances.js`, `admin.js` | Optional. If set, balances are gated behind the owner login; also the fallback for `ADMIN_SECRET` as the API automation credential (shortcuts); `/admin` itself is Google-login only. |
| `ADMIN_SECRET` | `admin.js` | Shared automation secret for API access from scripts/shortcuts (never a way into the `/admin` page, which is Google-login only). Falls back to `DASHBOARD_SECRET`; if neither is set, the admin APIs refuse to run. |
...
| `MERIDIAN_SPECIALS_KEY` | `special.js` | Base64 PKCS8 DER X25519 private key that decrypts MERIDIAN's special editions. The matching public key is committed in the dailymag repo (`state/specials-pubkey.b64`); the same value also lives as a GitHub Actions secret on dailymag for the reMarkable delivery workflow. |
...
- **Birthdays** — edit at **`/admin`** (writes the encrypted `data/birthdays.enc.json`; owner-only on the site). Each entry is `{ name, month, day }`. They appear in the For Mark section and in the personal calendar as small annual dot items.
