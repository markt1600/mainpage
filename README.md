...
- **`api/assist.js`** — the admin page's Claude calendar assistant: takes free-form text and/or a pasted screenshot plus both event lists, returns proposed adds (routed public/private) and deletions for review. Nothing commits until the owner confirms — confirming saves the ticked changes straight to the relevant list(s); the manual Save buttons are only for edits made directly in the tables. Uses `ANTHROPIC_API_KEY`.
