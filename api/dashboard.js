...

{
  ...
  "facts": [
    { "subject": "person's name", "fact": "1-2 sentence genuinely interesting, accurate, lesser-known fact about them" }
  ],
...
For "facts", provide 2-3 entries. Each must be an accurate, well-documented, genuinely interesting (ideally lesser-known) fact about ONE of these people: Haruki Murakami, Carl Sagan, Stephen Hawking, Kurt Cobain, or Bon Jovi (the band or Jon Bon Jovi). Pick a different mix of these people and different facts each day for variety. Draw these from your own knowledge — you do NOT need to web search for them. Keep each fact to 1-2 sentences.`;
...
async function getEdition(dateStr) {
  ...
  return {
    ...
    facts: Array.isArray(parsed.facts) ? parsed.facts.slice(0, 3).map(cleanFact) : [],
...
const FALLBACK_EDITION = {
  ...
  facts: [
    { subject: "Carl Sagan", fact: "He wrote the novel Contact, later adapted into the 1997 film, and helped design the golden records carried aboard the Voyager probes." },
    { subject: "Haruki Murakami", fact: "He ran a Tokyo jazz bar called Peter Cat before becoming a writer, and is a dedicated marathon runner." },
  ],
