...
// Verified facts run as a separate, web-search-grounded call.
...
export default async function handler(req, res) {
  ...
  // Weather, markets, and verified facts run independently so one failing
  // never blanks the others. Facts complete before the news call below, which
  // helps keep the two web-searching Claude calls from spiking at once.
  const [citiesR, marketsR, factsR] = await Promise.allSettled([getCities(), getMarkets(), getFacts()]);
  const cities = citiesR.status === "fulfilled" ? citiesR.value : [];
  const markets = marketsR.status === "fulfilled" ? marketsR.value : [];
  const facts = factsR.status === "fulfilled" ? factsR.value : FALLBACK_FACTS;
