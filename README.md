...
- **`api/networth.js`** — net worth for the For Mark section, encrypted at rest (`data/networth.enc.json`); the code deliberately contains no labels or amounts. Components are fixed amounts in SGD/USD or market rows (`symbol × qty`, valued at the live Yahoo price); the store also holds daily value snapshots (recorded on the first owner load of each SGT day) that power the 1D / month / YTD changes, plus a breakeven setting that drives the surplus/deficit line. Auth is **owner session only** — the shared admin secret is not accepted here, since it also lives in shortcut URLs. Edited in `/admin`'s two-currency panel with live conversion and a running total.
